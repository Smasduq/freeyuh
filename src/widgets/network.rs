...
pub fn create(app: &Application) -> (Button, Label) {
    ...
    wifi_window.set_default_size(390, 520);
    ...
    title_icon.set_markup("<span color=\"#a4d1b4\" size=\"large\">󰤨</span>");
    title_icon.add_css_class("wifi-title-icon");
    header.append(&title_icon);

    let title_box = Box::new(Orientation::Vertical, 1);
    title_box.set_halign(Align::Start);
    let title = Label::new(Some("Wi-Fi Networks"));
    title.add_css_class("wifi-header-title");
    title.set_halign(Align::Start);
    let subtitle = Label::new(Some("Wireless Connections"));
    subtitle.add_css_class("wifi-header-subtitle");
    ...
    rescan_btn.set_label("󰑐 Scan");
    rescan_btn.add_css_class("wifi-rescan-btn");
    ...
    status_label.add_css_class("wifi-status-label");
...
/// Populates `list_box` with restyled Wi-Fi network cards.
fn populate_list(
    list_box: &Box,
    ...
    if networks.is_empty() {
        ...
        let empty_sub = Label::new(Some("Click Scan to search again"));
    ...
    for net in networks {
        let item_card = Box::new(Orientation::Vertical, 4);
        ...
        if net.is_connected {
                        ...
                        Ok(res) => {
                            match res {
                                Ok(_) => {
                                    ...
                                    populate_list(&list_b_cb, &nets, &status_lbl_cb, &pill_lbl_cb);
                                }
                                Err(e) => {
            ...
            let password_container = Box::new(Orientation::Horizontal, 6);
